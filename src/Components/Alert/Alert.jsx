...
export default function Alert({ data, funct }) {
    ...
    useEffect(() => {
        setTimeout(() => {
            alertRef.current.classList.add('alert-active');

            if (data && data.time !== 'infinite') {
                setTimeout(() => {
                    alertRef.current.classList.remove('alert-active');
                    setTimeout(() => {
                        funct(null);
                    }, 200)
                }, data.time);
    ...
    return (
        <span className={`f-row f-justify-between f-align-center alert fixed br-6 alert-active`} ref={alertRef}>
